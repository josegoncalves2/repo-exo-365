...
#
#   1. NUCLEO (br.pmo.dlp) — JDK puro, zero dependencia externa. Compila e se
...
#
#   2. ADAPTADOR (br.pmo.dlp.exo) — precisa das classes da plataforma. Compila
...
#
# Saida: extensao/dlp-br/target/dlp-br.jar
# ===================================================================
set -euo pipefail
...
javac -Xlint:all -Werror \
      -encoding UTF-8 \
      -d "${CLASSES_NUCLEO}" \
      $(find "${AQUI}/src/br/pmo/dlp" -maxdepth 1 -name '*.java')
...
docker run --rm --user "$(id -u):$(id -g)" --memory=768m \
  ...
  sh -euc '
    ...
    cd /tmp/classes && jar --create --file /ext/target/dlp-br.jar .
    chmod 0644 /ext/target/dlp-br.jar
  '
...
for exigido in \
  ...
  "br/pmo/dlp/exo/ConectorDlpRegex.class" \
  "br/pmo/dlp/RegrasSensiveis.class" \
  "br/pmo/dlp/Varredura.class"
do
  if ! docker run --rm -v "${SAIDA}:/t:ro" "${IMAGEM}" \
        sh -c "unzip -l /t/dlp-br.jar 2>/dev/null | grep -q '${exigido}'"; then
...
echo "==> pronto: ${SAIDA}/dlp-br.jar"
ls -l "${SAIDA}/dlp-br.jar"
