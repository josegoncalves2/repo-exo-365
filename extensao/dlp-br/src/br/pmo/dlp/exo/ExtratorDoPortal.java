 ...
 * jars da plataforma.
 *
 * <p>Substitui {@code ExtratorTika} desta mesma extensao, que ficou no
 * repositorio sem ser referenciado. A diferenca nao e' estilo: aquele passava o
 * Content-Type do cliente como dica, devolvia string vazia para digitalizacao e
 * nao tinha defesa contra bomba de descompressao. Os tres pontos estao
 * resolvidos abaixo e cada um esta' justificado.
 *
  ...
  public String extrair(InputStream entrada, String nomeArquivo, String tipoMime)
    ...
    try (TikaInputStream fluxo = TikaInputStream.get(entrada)) {
      ...
      if (nomeArquivo != null && !nomeArquivo.isEmpty()) {
        // Tika 1.28.4: a chave vive em TikaMetadataKeys, nao em
        // TikaCoreProperties (onde ela so' aparece a partir da 2.x). Lido do
        // bytecode do jar da plataforma, nao suposto pela documentacao.
        metadados.set(TikaMetadataKeys.RESOURCE_NAME_KEY, nomeArquivo);
      }
