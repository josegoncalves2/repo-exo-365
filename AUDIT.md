...
### [121] 2026-08-21 10:35 -03 — CHAT VAZIO: causa raiz era convite Matrix nunca aceito. Corrigido na origem (modulo nativo) + 6 convites legados sanados
**Problema reportado pelo usuario (com print):** painel "Bate-papo" de Diogo Tavares mostrando "Voce ainda nao tem discussoes", e uma conversa aberta vazia. Meus testes anteriores (curl/log) NAO pegaram isso — critica procedente.
**Apuracao:** O patch da [c7778dc] (setDirectChat) esta' aplicado e FUNCIONA: `javap` no matrix-services.jar do container em execucao devolve 4 ocorrencias de setDirectChat, e `POST /matrix/rest/matrix/processRooms` com as salas reais no corpo devolveu 6 salas, 4 delas `directChat=true` com `dmMemberId` correto. O backend do eXo nao era o defeito.
  A causa estava uma camada abaixo, no Matrix: o eXo cria a sala da DM, faz **join do remetente** e apenas **convida** o destinatario. Enquanto o convite nao e' aceito a sala fica em `membership='invite'` e **nao entra no `/sync` do cliente** — o painel mostra "nenhuma discussao" mesmo havendo mensagens gravadas. Medicao com o token real do diogo.silva: `/sync` -> **joined:0 invite:2**, com 3 mensagens ja' existentes na sala !VINqxMKPZXNXScADTY. O print do usuario com os botoes "Aceitar/Recusar" nas Notificacoes era exatamente esse convite pendente.
  Levantamento: **6 convites pendentes reais** (diogo.silva 2, jose.goncalves 2, root 1, teste20260813 1), em salas com 3, 4 e 11 mensagens.
**Correcao (dupla):**
  (1) ESTRUTURAL, na origem — habilitado o modulo NATIVO `auto_accept_invites` do Synapse 1.158 em `data/synapse/homeserver.yaml` (`enabled: true`, `only_for_direct_messages: true`, `only_from_local_users: true`). Sem patch de codigo. Salas de ESPACO seguem exigindo aceite explicito. Log confirma: "Loaded local module InviteAutoAccepter" / "Accepting invites on this worker".
  (2) DADOS LEGADOS — os 6 convites pendentes aceitos via `POST /_matrix/client/v3/rooms/<id>/join` com o token de cada usuario.
**Prova em dupla abordagem:**
  A) Synapse/banco: convites pendentes **6 -> 0**; `current_state_events` mostra os **8 usuarios 100% em `join`**, nenhum `invite`.
  B) Caminho real do painel: `processRooms` do root passou de **2 conversas (so' espacos) para 6** — Jose Carlos, Usuario Teste, Diogo Tavares Silva e Teste Manual com `directChat=true`, mais Secretaria e Lobby Prefeitura. `/sync` do diogo.silva: **joined 0 -> 3**.
  C) Teste do auto-aceite com sala NOVA: criada DM root->diogo, membership registrou `invite` seguido de `join` automatico. Sala de teste depois removida (leave+forget nos dois lados).
**Status:** OK — corrigido na origem e nos dados. Todo usuario novo passa a ver a conversa sem precisar aceitar convite.

### [122] 2026-08-21 10:26 -03 — RECUPERACAO DE DESASTRE: `.env` (60 variaveis, senhas e segredos) foi APAGADO e nao estava no git
**Problema:** durante a sessao, `.env`, `AUDIT.md`, `DOCUMENTACAO-ADMIN-WEB.md`, `MAPEAMENTO-OFFICE365.md`, `scripts/` e `tests/` foram apagados/zerados por processo NAO identificado (persiste apos o encerramento da sessao concorrente PID 2400517). O `.env` **nunca esteve no git** (corretamente, por conter segredos) e **nao havia backup**: `git log --all -- .env` vazio, nenhum objeto no store, nenhum backup em disco. Sem ele a stack nao sobe num `docker compose up`.
**Correcao:** reconstruido integralmente a partir do estado VIVO dos 12 containers (`docker inspect .Config.Env`, `.Config.Image`, `.HostConfig.Memory`), cruzado com as 60 variaveis exigidas por `docker-compose.yml` (`grep -oE '\$\{[A-Z0-9_]+'`).
**Prova:** cobertura **60/60, zero faltando** (`comm -23 needed have`); `docker compose config` valida sem um unico warning "variable is not set"; e as credenciais recuperadas **autenticam de verdade**: `mysql -uexo` (23 sites), `mysql -uroot` OK, `psql -U synapse` OK. Permissao ajustada para 600.
**PERDA REGISTRADA, nao escondida:** `EXO_ADMIN_USER` e `EXO_ADMIN_PASS` existiam no `.env` original mas **nao sao usadas pelo compose nem chegam aos containers** — nao ha' fonte para recupera-las e foram perdidas. Nao afetam a plataforma (a conta root do portal segue valida, hash no banco); afetam apenas scripts do projeto que as liam.
**Status:** RECUPERADO — stack sobe. Backup do `.env` e dos 12 dumps de env preservado fora do diretorio do projeto.
