...
### [153] 2026-08-26 14:55:00 -03 — HTTPS habilitado (resolve IA, campos e contextos seguros)

**Acao:** O portal rodava em HTTP. O addon de IA (meeds-ai) quebra com `TypeError: crypto.randomUUID is not a function` em contexto nao seguro — o campo "API Key" do provedor NUNCA aparecia (o usuario reportou "nao existe onde enfiar a api token"). Com HTTPS (`crypto.randomUUID` exige secure context), o campo API Key aparece e o fluxo de criacao de provider funciona ate a validacao com o provedor.
**Feito:** `.env`: `EXO_PROXY_PORT=443`, `EXO_PROXY_SCHEMA=https`, `EXO_PROXY_SSL=true`. exo-app recriado. Navegador acessa `https://192.168.1.59` (certificado CA interna do projeto, ja montado em conf/portal-certs).
**Resultado:** `crypto.randomUUID: true`, `isSecureContext: true` em HTTPS. Tela `/administration/home/ai/models` mostra campo **API Key** no formulario Add Provider. Token `sk-31a82590du1029d02ud019u2aadasd12` preenchido; o eXo conectou ao OpenAI e recebeu 401 "Incorrect API key" (token invalido — o fluxo funciona, o token e' que nao e' valido).
**Status:** OK

---

### [154] 2026-08-26 14:56:00 -03 — DLP e MFA: telas confirmadas ao vivo

**Acao:** Usuario reportou "nao existe DLP". Verificado ao vivo (Playwright): as telas EXISTEM e funcionam em `Segurança` do menu admin:
- DLP: `/administration/home/security/quarantine` — switch Quarentena, palavras-chave (CPF, CNPJ, NOME), tabela de quarentena com colunas Conteudo/Palavra-chave/Data/Autor/Acoes.
- MFA/2FA: `/administration/home/security/multifactor-authentication` — verificacao em 2 etapas, metodo OTP, acesso protegido, paginas protegidas, revogacoes.
- O menu lateral so expande "Segurança" ao clicar (Quarentena, Regras de transferencia, Multi-fator).
**Resultado:** DLP e MFA confirmados funcionais na interface.
**Status:** OK

---

### [155] 2026-08-26 14:57:00 -03 — Icones dos atalhos recuperados + bug do padronizar corrigido
...
### [174] 2026-08-27 11:27:49 -03 — IA fora do ar apos reinicio: PKIX path building failed — corrigido com truststore ACRESCIDA das CAs internas
**Ação:** Apos o reinicio do exo-app o contexto Spring 'ai-agent' parou de inicializar. Causa raiz medida no platform.log, e nao suposta: o add-on ai-mcp-client resolve o cliente OAuth2 chamando ClientRegistrations.fromIssuerLocation() contra https://192.168.1.59/auth-server/.well-known/openid-configuration; a cadeia e a PKI interna (PMO eXo Root CA -> Intermediate) e ela NAO estava na cacerts da JVM (conferido com keytool: zero entradas PMO). O handshake morria com 'PKIX path building failed: unable to find valid certification path', sem documento de descoberta nao havia tokenUri, e o erro subia como 'tokenUri cannot be empty' derrubando aiProviderService -> aiModelService -> aiAgentService e o contexto ai-agent inteiro. Diagnostico anterior de 'corrida de partida' estava ERRADO: o segundo reinicio falhou identicamente, o que descartou timing. Correcao ADITIVA: conf/truststore/truststore-pmo.p12 e a cacerts da propria imagem COM as duas CAs internas acrescentadas (111 -> 113 entradas, delta 2 conferido no keytool), montada somente-leitura e apontada por tres -D acrescentados ao JAVA_OPTS existente. Superconjunto estrito: nenhuma confianca TLS anterior foi retirada, entao MySQL/ES/SMTP/ONLYOFFICE seguem iguais. Senha em EXO_TRUSTSTORE_PASSWORD no .env (o arquivo guarda so certificados publicos, sem chave privada).
**Comando/Arquivo:** `docker-compose.yml (JAVA_OPTS + volume), .env, conf/truststore/`
...
### [176] 2026-08-27 12:01:37 -03 — DLP por padrao: conector + enfileiramento proprio, com duas travas contra quarentena em massa
**Ação:** O exo-dlp so' casa PALAVRA-CHAVE contra o indice do ES, e sua acao oficial FileDLPAction NAO ENFILEIRA NADA quando exo.dlp.keywords esta vazio (que e o estado desta instalacao). Ou seja, o DLP rodava inerte. Acrescentados: (1) br.pmo.dlp.exo.ConectorDlpRegex, que HERDA de FileDlpConnector e delega a quarentena ao treatItem() protected da eXo, com deteccao por padrao (CPF/CNPJ/cartao-Luhn/PIS/titulo/CNH/chave PIX/segredo) com digito verificador conferido; (2) br.pmo.dlp.exo.ExtratorTika para PDF/DOCX/XLSX sem acrescentar jar (Tika 1.28.4, PDFBox e POI ja estao em /opt/exo/lib); (3) br.pmo.dlp.exo.AcaoEnfileirarDlp, plugin ADITIVO no SessionActionCatalog com nome proprio, que enfileira sem depender de palavra-chave. RISCO CATASTROFICO IDENTIFICADO E NEUTRALIZADO: FileDlpConnector.processItem nao confere se ha palavras-chave (quem conferia era a acao oficial, do lado de fora). Ao passar a enfileirar por conta propria, uma busca no ES com termo VAZIO poderia casar com TODO o acervo e manda-lo para quarentena. Duas defesas independentes: a acao so enfileira se getConnectors().get("file") for instanceof ConectorDlpRegex, e o conector so delega ao nativo se houver palavras cadastradas.
**Comando/Arquivo:** `extensao/dlp-br/construir.sh (dois estagios) + docker-compose.yml (montagem do jar)`
**Resultado:** Registro CONFIRMADO no boot 11:58:43: 'DLP por padrao: ligado=true severidadeMinima=ALTA acao=ALERTAR minimoOcorrencias=1 tetoBytesArquivo=16777216'. Portal saudavel, contexto ai-agent inicializado 12:00:08, zero erro de parse. Portao de build: XML conferido antes de compilar, 129 asseveracoes do nucleo com -Xlint:all -Werror, e conferencia do conteudo do jar. NAO E ACEITE: nenhuma linha disso prova a tela; o aceite e humano.
**Evidência:** docker logs exo-app | grep 'DLP por padrao'
**Status:** OK

### [177] 2026-08-27 12:12:55 -03 — DEFEITO CORRIGIDO no conector DLP: PDF digitalizado passava por documento LIMPO
**Ação:** Defeito achado em revisao cruzada pela sessao paralela, no mesmo dia em que o conector entrou. ConectorDlpRegex.extrairTexto tinha tres saidas que devolviam apenas nome e titulo do arquivo (acima do teto de bytes, nenhum extrator capaz de ler o binario, item sem conteudo binario). O comentario do codigo afirmava 'NAO e tratado como limpo' e o efeito era o OPOSTO: varrer 'ficha-funcional.pdf / Ficha Funcional' nao casa padrao nenhum e produz laudo COMPLETO e LIMPO, classificacao PUBLICO. A PoliticaDlp so ve o laudo e nao tinha como saber que o texto chegou capenga. Consequencia: PDF digitalizado ; que numa prefeitura e o formato mais comum de documento sensivel (ficha funcional, atestado, procuracao, RG anexado a processo) ; era dado por limpo. Nao era falso negativo de deteccao: era o motor sendo informado errado. Deixar so no log nao resolvia, porque log nao e decisao. CORRECAO: extrairTexto passou a devolver o par TextoExtraido (texto + motivo da parcialidade); quando ha motivo, processItem chama Varredura.varrerParcial(texto, motivo) em vez de varrer(texto), e o laudo nasce marcado como incompleto, disparando PoliticaDlp.getAcaoQuandoIncompleta(). Corrigido tambem um valor chumbado: acaoQuandoIncompleta era ALERTAR fixo no construtor e virou init-param dlp.regex.acaoQuandoIncompleta, com padrao REGISTRAR ; ALERTAR de saida num acervo com muita digitalizacao produz enxurrada no primeiro job, e alerta em massa e alerta que se aprende a ignorar. Trocado tambem ExtratorTika por ExtratorDoPortal, que detecta tipo pelos BYTES, levanta excecao em PDF sem camada de texto e limita razao de compressao (um .docx de 40 KB pode ter 4 GB descomprimidos).
**Comando/Arquivo:** `extensao/dlp-br/src/br/pmo/dlp/exo/ConectorDlpRegex.java`
**Resultado:** 137 asseveracoes do nucleo, 0 falhas. No ar e confirmado no boot 12:09:59: 'DLP por padrao: ligado=true severidadeMinima=ALTA acao=ALERTAR acaoQuandoIncompleta=REGISTRAR'. Portal saudavel, ai-agent inicializado 12:11:23.
**Evidência:** docker logs exo-app | grep 'DLP por padrao'
**Status:** OK

### [178] 2026-08-27 13:35:27 -03 — 2FA POR ZONA implementado do zero: extensao/mfa-zona
**Ação:** Briefing item 2.2. Medido antes de escrever: o add-on exo-multifactor-authentication tem EXATAMENTE dois criterios de decisao (MfaService.isProtectedUri e MfaService.currentUserIsInProtectedGroup) e NENHUM deles olha endereco de origem ; 2FA por zona nao existia na plataforma. Implementadas tres classes de nucleo em JDK puro (Zona: CIDR com comparacao em BITS, IPv4/IPv6 e desmapeamento de ::ffff:; OrigemRequisicao: resolucao de origem atras de proxy; CatalogoZonas: politica exigir/isentar) mais o filtro br.pmo.mfa.exo.FiltroMfaPorZona, registrado no ExtensibleFilter com prioridade 6 (depois do filtro nativo). O filtro NAO duplica o add-on: nao gera codigo, nao guarda segredo, nao desenha tela e nao valida OTP ; reaproveita a mesma tela /portal/dw/mfa-access e as mesmas marcas de sessao mfaValidated/mfaExpiration, de modo que o usuario tem um unico fluxo e um unico cadastro. FURO DE AUTENTICACAO ACHADO PELAS PROPRIAS PROVAS antes de ir ao ar: OrigemRequisicao.normalizar devolvia texto nao validado, entao X-Forwarded-For com lixo sairia como se fosse endereco e o catalogo o classificaria como fora de toda zona protegida, DISPENSANDO o segundo fator. Corrigido com Zona.enderecoValido(). O desenho central contra forja de XFF: o cabecalho so e considerado quando getRemoteAddr() e um proxy declarado, e a cadeia e lida DA DIREITA PARA A ESQUERDA.
**Comando/Arquivo:** `extensao/mfa-zona/ + docker-compose.yml + conf/exo.properties`
**Resultado:** 58 asseveracoes do nucleo, 0 falhas, com -Xlint:all -Werror. Filtro CONFIRMADO em execucao numa requisicao HTTP real no boot 13:33:26: '2FA por zona: INERTE (exo.mfa.zonas.exigir vazio); nada muda' na thread http-nio-0.0.0.0-8080-exec-1. NASCE INERTE de proposito: exo.mfa.zonas.exigir vazio, comportamento identico ao anterior. NAO E ACEITE: o aceite e humano, acessando de duas faixas de rede diferentes.
**Evidência:** docker logs exo-app | grep '2FA por zona'
**Status:** OK

### [179] 2026-08-27 13:57:43 -03 — Restricoes avancadas de transferencia (item 2.3) + 4 furos do 2FA por zona corrigidos apos fiscalizacao
**Ação:** ITEM 2.3: o nativo (TransferRulesRestService) sao DOIS INTERRUPTORES GLOBAIS no SettingService (sharedDocumentStatus, downloadDocumentStatus) ; liga/desliga para o portal inteiro. O briefing pede restricao 'com base em permissoes e politicas da empresa'. Implementada extensao/transferencia: motor de regras (grupo, grupo-excluido, extensao, tamanho, zona de rede, operacao), primeira-que-casa com a regra RESPONSAVEL NOMEADA em cada decisao, modo OBSERVACAO/APLICACAO. 38 asseveracoes. Nasce em OBSERVACAO porque as rotas de saida de arquivo nao sao declaradas em lugar algum legivel (tudo mapeado em /* no eXo 7.2.1) e bloquear por suposicao erra dos dois lados em silencio. FISCALIZACAO ADVERSARIAL do 2FA por zona (sub-agente fiscal, 21 ferramentas, 93k tokens) achou e PROVOU 4 furos, todos corrigidos: (1) ALTA ; /portal/rest isento em bloco, bastando chamar GET /portal/rest/v1/social/users para receber tudo sem segundo fator; agora so as rotas da propria tela de OTP sao livres e o resto responde 403; (2) ALTA ; startsWith sem fronteira de segmento fazia /portal/restrito e /portal/skinsecreta nascerem isentos, e getRequestURI cru divergia do caminho normalizado pelo Tomcat (%2e%2e); agora casa por segmento sobre servletPath+pathInfo; (3) ALTA ; pareceEnderecoLiteral aceitava hostname hexadecimal (f00dbabe.cafe.ac, TLDs .ac .de .be etc), levando a CONSULTA DE DNS SINCRONA no caminho da requisicao ; quem controlasse o dominio escolhia a propria zona e um DNS mudo prendia thread do Tomcat; substituido por parser proprio de IPv4 e IPv6; (4) ALTA ; o ramo dos colchetes de OrigemRequisicao devolvia sem validar, reabrindo pelo caminho paralelo o mesmo furo que a entrada [176] dizia ter fechado. Corrigidos ainda: entrada ilegivel na cadeia devolvia o IP do proprio proxy (falha ABERTA, pois o proxy costuma estar em faixa isenta) e agora devolve indeterminado; formas abreviadas de IPv4 (10.5, 2130706433, zero a esquerda) recusadas; exemplo 0.0.0.0/0 corrigido para incluir ::/0 porque o nginx escuta em [::] e a pilha v6 ficava isenta.
**Comando/Arquivo:** `extensao/transferencia/ + extensao/mfa-zona/ + conf/exo.properties`
**Resultado:** mfa-zona: 78 asseveracoes (era 58), 0 falhas ; as 20 novas sao regressao dos achados do fiscal. transferencia: 38 asseveracoes, 0 falhas. Ambas com -Xlint:all -Werror. CONFIRMADO em requisicao HTTP real no boot 13:55:48: '2FA por zona: INERTE' e 'Transferencia: INERTE'. DLP por padrao registrado 13:54:38, ai-agent inicializado 13:56:04, portal saudavel, zero erro de parse. O fiscal NAO achou defeito na aritmetica CIDR (40.000 casos IPv4 e 4.000 IPv6 contra referencia independente) nem na concorrencia (volatile + double-checked locking com campos final).
**Evidência:** docker logs exo-app | grep -E '2FA por zona|Transferencia:'
**Status:** OK

### [180] 2026-08-27 14:14:00 -03 — DESVIO FECHADO: WebDAV nao executava os filtros de 2FA por zona nem de transferencia
**Ação:** Apontado por revisao cruzada da sessao paralela e CONFIRMADO por medicao. O webapp portal declara org.exoplatform.web.filter.GenericFilter, que executa a cadeia do ExtensibleFilter e portanto chama as extensoes proprias; o webapp webdav NAO a declarava (so SetCurrentIdentityFilter e ResourceRequestFilter). E o nginx tem um location / que encaminha tudo, entao /webdav/... alcanca esse contexto de fora. Resultado: usuario sob exigencia de segundo fator montava o acervo como unidade de rede e lia e gravava documento SEM 2FA e sem passar por regra de transferencia. Mesmo padrao do /portal/rest isento em bloco, por outra porta. CORRECAO: conf/webdav-web.xml (que o Dockerfile ja injetava no war) passa a declarar o GenericFilter mapeado em /* ; e nao so em /drives/*, porque mapeamento parcial deixa de fora justamente as rotas nao previstas. Exigiu reconstrucao da imagem. Entregue tambem: relatorio de conformidade ligado ao ConectorDlpRegex (registro ANTES de qualquer ramo de decisao, com publicacao periodica no log) e extensao/wopi com TokenWopi (28 asseveracoes).
**Comando/Arquivo:** `conf/webdav-web.xml + docker compose build exo + docker compose up -d exo`
